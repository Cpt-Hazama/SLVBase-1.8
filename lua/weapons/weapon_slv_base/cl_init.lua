...
function SWEP:PlayThirdPersonAnim(anim)
	...
	if !SinglePlayer() || self.Owner:IsNPC() then self.Owner:SetAnimation(anim); return end
...
function SWEP:ViewModelDrawn()
	if self.CModel then
		local bValid = ValidEntity(self.clMdl)
		if !bValid then
...
usermessage.Hook("HLR_SWEPDoReload", function(um)
	...
	if !ValidEntity(wep) || LocalPlayer():GetActiveWeapon() != wep then return end
	wep.Weapon:Reload()
end)

usermessage.Hook("slv_tpanim", function(um)
	local ent = LocalPlayer():GetActiveWeapon()
	if !ValidEntity(ent) || !ValidEntity(ent.Owner) then return end
