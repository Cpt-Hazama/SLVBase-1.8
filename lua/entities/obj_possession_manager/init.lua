...
local schdRunToLastPosition = ai_schedule.New("RnLP")
...
local schdWalkToLastPosition = ai_schedule.New("WlkLP") 
...
function ENT:StartPossession()
	...
	local tblNPCs = list.Get("NPC")
	local class = self.entTarget:GetClass()
	umsg.Start("HLR_Possessor_PossessionStart", self.entPossessor)
		umsg.String(tblNPCs && tblNPCs[class] && tblNPCs[class].Name || class)
		umsg.Short(self.entTarget:GetMaxHealth())
		umsg.Short(self.iHealthTarget)
	umsg.End()
	self.entPossessor:DisableAUXPowers()
	...
	self.tblPlayer["ActiveWeapon"] = self.entPossessor:GetActiveWeapon():GetClass()
...
function ENT:EndPossession()
	if ValidEntity(self.entPossessor) then
		umsg.Start("HLR_Possessor_PossessionEnd", self.entPossessor)
		umsg.End()
		...
		if ValidEntity(self.entTarget) then
		...
		self.entPossessor:SelectWeapon(self.tblPlayer["ActiveWeapon"])
		self.entPossessor:SetHealth(self.tblPlayer["Health"])
		self.entPossessor:SetArmor(self.tblPlayer["Armor"])
		self.entPossessor:EnableAUXPowers()
	end
	
	if ValidEntity(self.entCam) then self.entCam:Remove() end
	if ValidEntity(self.entTarget) then
...
function ENT:TargetFaceForward(fcDone)
	...
	local schdFaceLastPosition = ai_schedule.New( "Face to last position" ) 
	...
	if self.entPossessor:KeyDown(iKey) && func then
		...
		//self:TargetFaceForward(function()
			...
			func(self.entTarget, self.entPossessor, function(bFailed)
				self.bInSchedule = false
				if !bFailed then self.entTarget:TaskComplete(); self:Think() end
			end)
		//end)
...
function ENT:Think()
	if !ValidEntity(self.entTarget) || !ValidEntity(self.entPossessor) || self.entPossessor:KeyDown(IN_USE) || self.entPossessor:Health() <= 0 then self:EndPossession(); return end
	local iHealth = self.entTarget:Health()
	if iHealth != self.iHealthTarget then
		self.iHealthTarget = iHealth
		umsg.Start("HLR_Possessor_UpdateHealth", self.entPossessor)
			umsg.Long(iHealth)
		umsg.End()
	end
	...
	self.bInSchedule = self:RunAttack(IN_ATTACK, self.entTarget._PossPrimaryAttack) || self:RunAttack(IN_ATTACK2, self.entTarget._PossSecondaryAttack) || self:RunAttack(IN_RELOAD, self.entTarget._PossReload) || self:RunAttack(IN_JUMP, self.entTarget._PossJump)
	...
	self.entTarget:SetAngles(Angle(0,math.ApproachAngle(ang.y,angTgt.y,4)))
