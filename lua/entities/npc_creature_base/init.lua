...
hook.Add("PlayerDeath","npckilledplayer",function(pl,inflictor,killer)
	if(ValidEntity(killer) && killer:IsNPC() && killer.OnKilledTarget) then
...
function ENT:GetEnemies()
	...
	for k, ent in pairs(tblPotentialEnemies) do
		...
		if (bNPC || bPlayer) && !ent:GetNoTarget() then
			...
			if bValid && fDist <= viewDist && self:IsInAttackRange(ent) && !ent.bSelfDestruct then
...
function ENT:GetPlayerOwner()
	return ValidEntity(self.m_plOwner) && self.m_plOwner || nil
end
...
function ENT:IsEnemy(ent)
	...
	return ValidEntity(ent) && !ent.bNeutral && (ent:IsNPC() || ent:IsPlayer()) && !ent:GetNoTarget() && self:Disposition(ent) <= 2 && (!ent:IsPlayer() || (!tobool(GetConVarNumber("ai_ignoreplayers")) && !ent:IsPossessing())) && ent != self && ent:Health() > 0 && gamemode.Call("ShouldIgnoreEnemy",self,ent) != true && (!ent.IsCompanion || !ent:IsCompanion()) && self:IsInAttackRange(ent)
end

// What's this for again?
function ENT:IsEnemyPlayer(ent)
	return ValidEntity(ent) && ((ent:IsNPC() && ent.Charmed && ent:Charmed()) || ent:IsPlayer()) && !ent:GetNoTarget() && self:Disposition(ent) <= 2 && (!ent:IsPlayer() || (!tobool(GetConVarNumber("ai_ignoreplayers")) && !ent:IsPossessing())) && ent != self && ent:Health() > 0
end
...
local schdTurnToEnemy = ai_schedule.New()
...
function ENT:UpdateMemory()
	...
	for ent, data in pairs(mem) do
		...
		if !bValid || ent:Health() <= 0 || self:OBBDistance(ent) > viewDist || !self:IsInAttackRange(ent) || ent:GetNoTarget() || (ent:IsPlayer() && (!ent:Alive() || tobool(GetConVarNumber("ai_ignoreplayers")))) || (iDisposition != 1 && iDisposition != 2) || (self:GetAIType() == 5 && ent:WaterLevel() < 2) || ent.bSelfDestruct then
			self:RemoveFromMemory(ent)
		end
	end
end
...
function ENT:UpdateEnemies()
	local num = self.iMemCount
	if self.entEnemy && (!ValidEntity(self.entEnemy) || self.entEnemy:Health() <= 0 || self.entEnemy:GetNoTarget() || !self:IsInAttackRange(self.entEnemy) || (self:GetAIType() == 5 && self.entEnemy:WaterLevel() < 2) || (self.entEnemy:IsPlayer() && (tobool(GetConVarNumber("ai_ignoreplayers")) || self.entEnemy:IsPossessing())) || self.entEnemy.bSelfDestruct) then self:RemoveFromMemory(self.entEnemy) end
	self:UpdateMemory()
	self:GetEnemies()
...
function ENT:DoRagdollDeath(dmginfo)
	...
	if ValidEntity(self.ragdoll) then
		...
		if !self.BodyCaps && iKeepRagdolls != 2 && !self.UseRagdollPhysicsProp then
...
function ENT:DoDeath(dmginfo)
	...
	if (!ValidEntity(attacker) || attacker:GetClass() != "npc_barnacle") && !dmginfo:IsDamageType(DMG_DISSOLVE) then
		...
		if self.bPlayDeathSequence then
			if((bRagdoll && tobool(GetConVarNumber("ai_keepragdolls"))) || self.UseRagdollPhysicsProp) then self:DoRagdollDeath(dmginfo)
			else
	...
	if(event == "play") then
		...
		if(sndEv) then
			if(string.Right(sndEv,4) != ".wav") then self:PlaySound(sndEv)//,true,true)
				...
				WorldSound(snd,self:GetPos(),75,self:GetSoundPitch())
			end
		end
		return true
	end
...
function ENT:Event(strEv)
	...
	if(!self:HandleDefaultEvents(event,unpack(args)) && !self:EventHandle(event,unpack(args))) then
