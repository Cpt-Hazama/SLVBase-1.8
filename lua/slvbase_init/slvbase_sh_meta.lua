	...
	hook.Add("PlayerInitialSpawn", "SLV_UpdateCollisions", function(ply)
		...
		timer.Simple(0.5, function() -- Let the entities initialize clientside
			...
			for entA, dat in pairs(tblEntsNoCollide) do
				if !ValidEntity(entA) then tblEntsNoCollide[entA] = nil
				else
			...
			for ent, dat in pairs(tblEntsNoCollideByClass) do
				if !ValidEntity(ent) then tblEntsNoCollide[ent] = nil end
			end
...
function meta:NoCollide(ent)
	...
	for ent, _ in pairs(tblEntsNoCollide[self]) do if !ValidEntity(ent) then tblEntsNoCollide[self][ent] = nil end end
...
function meta:Collide(ent)
	if type(ent) == "string" then
		if tblEntsNoCollide[self] then
			for _ent, _ in pairs(tblEntsNoCollide[self]) do
				if ValidEntity(_ent) && _ent:GetClass() == ent then
	...
	if tblEntsNoCollide[self] then for ent, _ in pairs(tblEntsNoCollide[self]) do if !ValidEntity(ent) then tblEntsNoCollide[self][ent] = nil end end end
...
function meta:IsPhysicsEntity()
	return !self:IsNPC() && !self:IsPlayer() && !self:IsWorld() && ValidEntity(self:GetPhysicsObject())
end
