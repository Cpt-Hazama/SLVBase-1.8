...
usermessage.Hook("ENT_FreezeEffect_Render_End", function(um)
	local ent = um:ReadEntity()
	if !ValidEntity(ent) || !table.HasValue(tbEntsFrozen, ent) then return end
	...
	if(ent == LocalPlayer()) then
		...
		//if ValidEntity(vmModel) then vmModel:Remove(); ent:GetViewModel():SetColor(255,255,255,255) end
	end
...
usermessage.Hook("ENT_FreezeEffect_Render", function(um)
	local ent = um:ReadEntity()
	if(!ValidEntity(ent) || table.HasValue(tbEntsFrozen,ent)) then return end
					...
					cam.Start3D(EyePos(),EyeAngles())
						...
						SetMaterialOverride(mat)
							...
							if(ent:IsPlayer()) then
								...
								if(ValidEntity(wep)) then wep:DrawModel() end
							end
...
usermessage.Hook("HLR_EntsCollide", function(um)
	...
	if !ValidEntity(entA) || !ValidEntity(entB) then return end
...
usermessage.Hook("HLR_EntsCollideClass", function(um)
	local entA = um:ReadEntity()
	local class = um:ReadString()
	if !ValidEntity(entA) || !class then return end
	local bCollide = um:ReadBool()
