local cvarThawTime = CreateConVar("sv_freeze_thawtime",20,FCVAR_ARCHIVE)
local cvarThawDelay = CreateConVar("sv_freeze_thawdelay",3,FCVAR_ARCHIVE)

local ai_scheduleNew = ai_schedule.New
...
function meta:SetFrozen(iPercent)
	...
	if(bNew) then
		umsg.Start("ENT_FreezeEffect_Render")
			umsg.Entity(self)
		umsg.End()
	end
end
...
function meta:UnFreeze(iPercent)
	...
	umsg.Start("ENT_FreezeEffect_Render_End")
		umsg.Entity(self)
	umsg.End()
...
hook.Add("Think", "HLR_CheckNPCRelationships_Except", function()
	for src, rel in pairs(tblEntRelationshipExcept) do
		if !ValidEntity(src) then tblEntRelationshipExcept[src] = nil
...
function meta:SetNoTarget(bNoTarget)
	...
	for ent, rel in pairs(tblNoTargetNPCs) do
		if !ValidEntity(ent) then tblNoTargetNPCs[ent] = nil end
	end
end

hook.Add("OnEntityCreated", "HLR_EntCreated_ApplyNoTarget", function(ent)
	if ValidEntity(ent) && ent:IsNPC() && !ent.bScripted then
		for _ent, rel in pairs(tblNoTargetNPCs) do
			if ValidEntity(_ent) then
...
function meta:SetFrozen(iPercent)
	...
	if(bNew) then
		...
		local entIndex = self:EntIndex()
		hook.Add("Think", "ThinkEntFrozen" .. entIndex, function()
			if !ValidEntity(self) then hook.Remove("Think", "ThinkEntFrozen" .. entIndex); return end
...
function meta:SetArcVelocity(vecStart,vecEnd,speed,dirRef,tolerance,dirOffset)
	...
	if(vecToss == vector_origin) then vecToss = self:GetForward() *speed *0.8 +self:GetUp() *speed *0.2
		...
		if bNPC && self.bScripted then
			...
			hook.Add("Think", "HLR_EntityIgniteThink", function()
				for ent, data in pairs(tblEntsIgnited) do
					if !ValidEntity(ent) || ent:Health() <= 0 || !ent.tblFlameEffects then
						data.cspBurn:Stop()
					...
					elseif CurTime() >= data.duration || ent:WaterLevel() == 3 then
						...
						if CurTime() >= data.nextDmg then
							local inflictor = ValidEntity(self) && self || ent
							data.nextDmg = CurTime() +0.2
							local dmg = DamageInfo()
							dmg:SetDamage(1)
							dmg:SetDamageType(DMG_DIRECT)
							dmg:SetAttacker(ValidEntity(attacker) && attacker || GetWorldEntity())
							dmg:SetInflictor(inflictor)
							ent:TakeDamageInfo(dmg)
							if !ent.m_bDontGetBurnt then
								local r, g, b, a = ent:GetColor()
								r = math.Clamp(r -8, 45, 255)
								g = math.Clamp(g -8, 45, 255)
								b = math.Clamp(b -8, 45, 255)
...
function meta:FadeOut(fDelay)
	...
	timer.Create("FadeOutTimer" .. index, 0.01, iLoops, function()
		if !ValidEntity(self) then timer.RemoveSafely("FadeOutTimer" .. index); return end
...
function meta:FadeIn(fDelay)
	...
	timer.Create("FadeInTimer" .. index, 0.01, iLoops, function()
		if !ValidEntity(self) then timer.RemoveSafely("FadeInTimer" .. index); return end
...
function meta:Dissolve(entAttacker, entInflictor, iType)
	...
	timer.Simple(0, function()
		if ValidEntity(entDissolver) then entDissolver:Remove() end
		if ValidEntity(self) then self:SetName(_sName) end
	end)
end
