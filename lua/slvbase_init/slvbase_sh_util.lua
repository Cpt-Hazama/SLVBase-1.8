...
function timer.RemoveSafely(tm)	-- Removing a non-simple timer within the same timer can break the whole module. This can prevent that.
	timer.Stop(tm)
	timer.Simple(0,timer.Remove,tm)
end
...
function util.CalcArcVelocity(ent,vecSpot1,vecSpot2,flSpeed,flTolerance)
	...
	if(tr.Fraction != 1) then return vector_origin end
	local tr = util.TraceLine({
		start = vecApex,
		endpos = vecSpot2,
